...
from explainshell import matcher, store, errors, helpconstants
...
class TestMatcherCoverage(unittest.TestCase):
    ...
    def test_visitreservedword_with_compound_context(self):
        ...
        with patch.dict(helpconstants.COMPOUNDRESERVEDWORDS, {"for": {"do": "for do help"}}):
    ...
    def test_visitoperator_with_compound_context(self):
        ...
        with patch.dict(helpconstants.COMPOUNDRESERVEDWORDS, {"if": {";": "if semicolon help"}}):
    ...
    def test_startcommand_program_not_found(self):
        ...
        m = matcher.matcher("", self.store)
        self.store.findmanpage.side_effect = errors.ProgramDoesNotExist("unknown")
    ...
    def test_startcommand_multicommand_success(self):
        ...
        word_node1 = MagicMock()
        word_node1.word = "git"
        word_node1.pos = [0, 3]
        word_node1.parts = []
        
        word_node2 = MagicMock()
        word_node2.word = "commit"
        word_node2.pos = [4, 10]
        word_node2.parts = []
    ...
    def test_visitword_nested_command_end(self):
        ...
        mg = matcher.matchgroup("command0")
        mg.manpage = MagicMock()
        m.groups.append(mg)
        m.groupstack.append((None, mg, ["\\;"]))  # End word list
    ...
    def test_visitword_short_option_splitting(self):
        ...
        def find_option_side_effect(opt):
            if opt == "-l":
                return mock_option_l
            elif opt == "-a":
                return mock_option_a
            return None
        
        mock_manpage.find_option.side_effect = find_option_side_effect
        mg.manpage = mock_manpage
        m.groups.append(mg)
        m.groupstack.append((None, mg, None))
    ...
    def test_visitword_previous_option_expects_arg(self):
        ...
        mg = matcher.matchgroup("command0")
        mock_manpage = MagicMock()
        mock_manpage.find_option.return_value = None  # Current word not an option
        mg.manpage = mock_manpage
        m.groups.append(mg)
        m.groupstack.append((None, mg, None))
        
        # Setup previous option that expects arg
        mock_prev_option = MagicMock()
        mock_prev_option.expectsarg = True
        mock_prev_option.nestedcommand = False
        m._prevoption = mock_prev_option
    ...
    def test_visitword_nested_command_option(self):
        ...
        # Setup previous option that can nest commands
        mock_prev_option = MagicMock()
        mock_prev_option.expectsarg = True
    ...
    def test_visitword_partial_match_success(self):
        ...
        # Setup command with partial matching enabled
        mg = matcher.matchgroup("command0")
        mock_manpage = MagicMock()
        mock_manpage.find_option.return_value = None
        ...
        with patch.object(m, 'find_option') as mock_find:
            mock_find.side_effect = [None, MagicMock(), MagicMock(), MagicMock()]  # First None, then options
    ...
    def test_visitfunction_compound_curly_braces(self):
        ...
        shell_results = [r for r in m.groups[0].results if "function" in r.text.lower()]
    ...
    def test_markunparsedunknown_comment(self):
        ...
        comment_matches = [r for r in m.groups[0].results if "comment" in r.text.lower()]
    ...
    def test_mergeadjacent_multiple_same_text(self):
        ...
        with patch.object(m, '_resultindex', return_value={matches[0]: 0, matches[1]: 1}):
            result = m._mergeadjacent(matches)
