...
class TestStoreCoverage(unittest.TestCase):
    ...
    def test_store_drop_none_collections(self):
        ...
        with patch("pymongo.MongoClient", return_value=self.mock_client):
            s = self._extracted_from_test_verify_none_collections_4()
            s.drop(confirm=True)
    ...
    def test_store_contains_true(self):
        ...
        with patch("pymongo.MongoClient", return_value=self.mock_client):
            ...
            self.mock_mapping.count_documents.assert_called_with(
                {"src": "test"}
            )
    ...
    def test_findmanpage_mapping_manpage_mismatch(self):
        ...
        self._extracted_from_test_findmanpage_with_section_sorting_4(1)
    ...
    def test_findmanpage_with_section_sorting(self):
        ...
        self._extracted_from_test_findmanpage_with_section_sorting_4(2)
        self.mock_manpage.find.return_value = [
            {"_id": "id1", "name": "test", "source": "test.1.gz"},
        ...
        with patch("pymongo.MongoClient", return_value=self.mock_client):
            with patch.object(
                store.store, "_discovermanpagesuggestions", return_value=[]
            ):
                s = store.store()
    ...
    def _extracted_from_test_findmanpage_with_section_sorting_4(self, arg0):
        self.mock_mapping.count_documents.return_value = 2
        self.mock_mapping.find.return_value = [
            {"dst": "id1", "score": 10},
            {"dst": "id2", "score": 5},
        ]
        self.mock_manpage.count_documents.return_value = arg0
    ...
    def test_discovermanpagesuggestions_no_suggestions(self):
        ...
        with patch("pymongo.MongoClient", return_value=self.mock_client):
            s = store.store()
            result = s._discovermanpagesuggestions(
                "id1", [("id1", MagicMock())]
            )
    ...
    def test_addmanpage_existing_manpage(self):
        ...
        self.mock_mapping.count_documents.side_effect = [
            5,
            3,
        ...
        with patch("pymongo.MongoClient", return_value=self.mock_client):
            self._extracted_from_test_addmanpage_existing_manpage_28(
                test_manpage
            )

    # TODO Rename this here and in `test_addmanpage_existing_manpage`
    def _extracted_from_test_addmanpage_existing_manpage_28(
        self, test_manpage
    ):
        s = store.store()
    ...
    def test_verify_unreachable_manpages(self):
        ...
        with patch("pymongo.MongoClient", return_value=self.mock_client):
            notfound = self._extracted_from_test_verify_missing_manpages_11()
            self.assertEqual(notfound, set())
    ...
    def _extracted_from_test_verify_missing_manpages_11(self):
        s = store.store()
        ok, unreachable, result = s.verify()
        self.assertFalse(ok)
        self.assertIn(type(unreachable), [list, set])
        return result
    ...
    def _extracted_from_test_verify_none_collections_4(self):
        result = store.store()
        result.mapping = None
        result.manpage = None
        return result
    ...
    def test_paragraph_cleantext(self):
        ...
        p = store.paragraph(
            0, "<b>bold</b> &lt;test&gt; text", "SECTION", False
        )
        result = p.cleantext()
    ...
    def test_option_initialization_with_nestedcommand(self):
        ...
        # Should raise assertion error if nestedcommand=True but
        # expectsarg=False
    ...
    def test_manpage_arguments_property(self):
        ...
        mp = store.manpage(
            "test.1.gz", "test", "synopsis", [opt1, opt2, opt3], []
        )
    ...
    def test_manpage_synopsisnoname_property(self):
        ...
        mp1 = store.manpage(
            "test.1.gz", "test", "test - does something", [], []
        )
