...
class option(paragraph):
    ...
    expectsarg - specifies if one of the short/long options expects an
        additional argument
    argument - specifies if to consider this as positional arguments
    nestedcommand - specifies if the arguments to this option can start
        a nested command"""
...
class manpage(object):
    ...
    paragraphs - a list of paragraphs (and options) that contain all of the
        text and options extracted from this man page
    aliases - a list of aliases found for this man page
    partialmatch - allow interperting options without a leading '-'
    multicommand - consider sub commands when explaining a command with
        this man page, e.g. git -> git commit
    updated - whether this man page was manually updated
    nestedcommand - specifies if positional arguments to this program can
        start a nested command, e.g. sudo, xargs
    """
    ...
    def from_store(cls, d):
        ...
        for pd in d.get("paragraphs", []):
            ...
            if pp.is_option and "short" in pd:
...
class store(object):
    ...
    def findmanpage(self, name):
        """find a man page by its name, everything following the last dot (.)
        in name, is taken as the section of the man page
        ...
        if self.manpage is not None:
            ...
            for alias, score in m.aliases:
                if alias not in self:
                    ...
                    logger.info(
                        'inserting mapping %s -> %s (%s) score %d',
                        alias,
                        m.name,
                        _id,
                        score,
