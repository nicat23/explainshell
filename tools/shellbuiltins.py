...
colon_desc = """No  effect;  the command does nothing beyond expanding
...
_add(
    ...
    [so(sp(0, colon_desc, "", True), [], [], False, True, False)],
)

source = textwrap.dedent(
    """       <b>.</b>  <u>filename</u> [<u>arguments</u>]
       <b>source</b> <u>filename</u> [<u>arguments</u>]
              Read  and  execute  commands  from <u>filename</u> in the current
              shell environment and return the  exit  status  of
              the  last  command executed  from  <u>filename</u>.  If <u>
              filename</u> does not contain a slash, filenames in <b>PATH</b>
              are used to  find  the  directory  containing <u>filename
              </u>.  The file searched for in <b>PATH</b> need not be
              executable.  When <b>bash</b> is not  in  <u>posix</u>
              <u>mode</u>,  the  current  directory  is searched  if no
              file is found in <b>PATH</b>.  If the <b>sourcepath
              </b> option to the <b>shopt</b> builtin command is turned
              off,  the  <b>PATH</b>  is  not searched.   If  any
              <u>arguments</u>  are  supplied,  they  become the positional
              parameters when <u>filename</u> is executed.  Otherwise
              the positional  parameters  are unchanged.  The return status is
              the status of the last command exited within
              the  script  (0  if  no commands  are  executed), and false if <u>
              filename</u> is not found or cannot be read."""
)
...
_add(
    ...
    [so(sp(0, break_desc, "", True), [], [], False, True, False)],
)

history_desc = """<b>history</b> <b>[</b><u>n</u><b>]</b>
<b>history</b> <b>-c</b>
...
_add(
    ...
    [
        so(sp(0, history_desc, "", True), [], [], False, True, False),
        so(
            sp(
                1,
